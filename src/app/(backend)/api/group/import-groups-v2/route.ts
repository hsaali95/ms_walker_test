  ...
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      const accesstype = accesstypes.find(
        (acc) => acc.name?.toLowerCase() === record.AccessType.toLowerCase()
      )!;
      const group = transformGroup(record, exited_users, accesstype);

      const team = await Group.create(group, {
    ...
    return successResponse(teams, "Group Imported successfully");
  } catch (error: any) {
    console.error("Error import Group:", error);
    return errorResponse("Failed to group Teams", 500);
  }
}
